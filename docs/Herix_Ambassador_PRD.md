...
23. [赫府定位（2026-07-06）](#23-赫府agency定位2026-07-06-简化)
...
- **数据模型**：`referral_records` 一行=一个被邀请用户，`UNIQUE(task_id, user_hash)` 按身份去重；行级 `settled_txn_id` 防重复打款（每个用户身份只结算一次，商家手滑免疫）
- **状态单向**：未出现→已注册→已转化；1 改回 0 不降级，已结算不回收
- **跨码冲突不静默**：同一用户出现在两个码下 → 该行不入库，响应 `conflicts` 列出（现归属码 vs 本次码），商家在上传结果里指定唯一归属并填写理由（`reassign` 端点，审计留痕）；**已结算行不可改归属**
...
## 23. 赫府（Agency）定位（2026-07-06 简化）

### 23.1 赫府在生态中的角色

赫府是代理公司或个人经纪人，手上已有一批赫使资源，同时代表品牌客户发布任务。

**平台对赫府的定位：普通商家账号，无需特殊角色或架构。**

赫府在 Herix 上的操作与品牌完全相同：注册商家账号 → 充值 → 发任务 → 审核报名 → 结算。赫府管理赫使的方式是**定向任务**（见第 24 节）：将任务私密发送给指定赫使，不走公开招募，平台不需要感知"谁是谁的赫使"。

| 角色 | 平台身份 | 核心用途 |
|------|---------|---------|
| 品牌 | BRAND 账号 | 公开任务为主 |
| 赫府 | BRAND 账号（同上） | 定向任务为主，管理自己的赫使网络 |
| 赫使 | HERALD 账号 | 接公开或定向邀请任务 |

> 不需要 `is_agency` 标记、不需要名册表、不需要特殊定价。前期赫府与品牌走完全相同的流程。
...
定向任务是**仅对指定赫使可见**的任务，不出现在公开任务列表中。品牌或赫府用它将任务直接分配给特定赫使，跳过公开招募环节。
