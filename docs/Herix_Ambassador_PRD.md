...
## 29. 商家侧合规控制（2026-07-18 立规则）

**背景**：商家是付费方（预充值模式），平台对商家的信用敞口和洗钱风险结构性小于收款型平台
（Amazon/乐天式 KYB 强制的场景是"替商家收钱"）。据此采用分层控制，不在上线期做全量 KYB：

| # | 规则 | 状态 |
|---|------|------|
| 1 | **退款只退原汇款账户（原路退回）**——堵"充值→退款换账户"的经典洗钱通道 | 协议 v3 已写入（二.4）；钱包→银行的资金流出通道尚未开发，开发时本规则为硬约束 |
| 2 | **暴排条款（反社会势力排除）**——日本 B2B 合同标配 | 协议 v3 已加第七条（自拟草稿，⚠️待日本律师审阅） |
| 3 | **KYB 分层**：注册/体验额度免 KYB（点击签约+邮箱验证+银行转账实名背书已够）；**提升信用额度（credit_limit_override）或未来大额累计充值必须先 KYB**（登記簿謄本/营业执照，admin 人工审核后置 `is_enterprise_verified=1`）——信用额度=平台授信，授信必须知道对方是谁 | ✅ 已实现（2026-07-18）：商家设置页上传证件即提交（`kyb_status` none/pending/approved/rejected），admin 审核中心「商家认证」tab 审批，通过=is_enterprise_verified=1+站内信 |
| 4 | 赫使侧 KYC（在留声明）已有 | 已上线 |
| 5 | **首任务审核门**（2026-07-18 加，防恶意注册用任务做钓鱼/违规内容载体——预付费模式下内容风险>资金风险）：未 KYB 商家（is_enterprise_verified=0）发布的任务 `platform_review='pending'`，状态照常 OPEN/额度照常占用，但**不进公开列表、直链对非创建者 404**；admin「任务审核」tab 通过后公开+站内信，拒绝则退回草稿附原因（发布无硬资金锁，退回零资金副作用）。**已 KYB 商家免审直接上线**——与信用额度门形成一致激励。行业对标：广告平台全量审、星图/蒲公英先认证后发布，此为最轻实现 | ✅ 已实现（2026-07-18） |

**协议版本史**：v1(2026-07-09 初版) → v2(2026-07-17 第五条数据处理委托条款) → v3(2026-07-18 第七条暴排+退款原路退回)。
改版本三处同步：users.ts `AGREEMENT_VERSION` + `merchant.agreement.docTitle` + `merchant.agreement.version` 词条。
**法务审阅清单（正式开客前）**：协议全文三语（尤其第五条 DPA、第七条暴排为自拟草稿）+ 签约流程（click-wrap 证据链）。
...
### 27.1 越南语术语（2026-07-19 用户拍板，范围=仅客户端/赫使端）

小程序/H5 赫使端新增越南语（vi），核心术语定版（原则同上：用目标用户圈子里已有的词，不造新词）：

| 概念 | 越南语 | 依据 |
|------|--------|------|
| 赫使 | **Đại sứ** | 越南 KOL 营销行业通行说法 đại sứ thương hiệu（品牌大使）的核心词，与日文アンバサダー/英文 Ambassador 同一本地化路线 |
| 任务（UI层） | **Nhiệm vụ** | 越南 App"做任务领奖励"场景的标准用词；营销层"案件"将来若需要另用 Chiến dịch（campaign），两层区分与日文体系对齐 |
| ⚡极速打款 | **⚡ Thanh toán nhanh** | 越南电商/支付场景标准词"快速支付"，简短适合角标 |
| 任务体验（类型B角标） | **Trải nghiệm** | "体验"，短词适合卡片角标 |
| 成果报酬（类型A角标） | **Hoa hồng** | 越南联盟营销（affiliate）里"佣金"的绝对通行词，做过推广的用户一眼懂 |

范围说明：商家后台（merchant.*）与 admin 不做越南语；涉及资金的客户端文案（提现/税务提示）初版为 AI 译文，标记待母语者复核。

### 27.2 韩文术语（2026-07-19 用户拍板，范围=仅商户端）

商家后台（merchant.html）新增韩文（ko），覆盖 merchant.* 全部键 + 页面引用的共享键（auth/common/category/taskCard 等，共577键），核心术语定版：

| 概念 | 韩文 | 依据 |
|------|------|------|
| 赫使 | **앰배서더** | 韩国营销行业标准词 브랜드 앰배서더（品牌大使），与日/英/越同一本地化路线 |
| 任务（商户端） | **캠페인** | campaign 的韩语通行词，韩国营销平台（Revu等）对商家的标准叫法，与日文商户端用「案件」的专业定位对齐 |
| 广告代理 | 광고 대행사 | 韩语广告代理公司标准词 |
| ⚡极速打款 | ⚡ 빠른 정산 | "快速结算"，韩国平台商业通行表述 |
| 任务体验/成果报酬 | 체험형 / 성과 보상 | 체험단(体验团)营销是韩国成熟品类，성과 보상=按成果计酬标准词 |
| 商家后台 | 비즈니스 콘솔 | Business Console 音译，Meta/LINE 命名惯例同路线 |
| 推广码/转化/结算 | 추천 코드 / 전환 / 정산 | 韩国 affiliate 行业标准词 |

范围说明：客户端（赫使端小程序/H5）与 admin 不做韩文；主页 index.html 同步含韩文（内嵌静态词典，2026-07-19 补）。⚠️已知漏斗缺口：HP 韩文访客点「앰배서더 되기」进入的客户端无韩文（回落中文/英文），客户端加 ko 是独立决策。协议条文（agreement.*）韩文为 AI 译文，正式面向韩国商家前须经法务/母语者复核。分叉守卫：checkKoParity()（merchant.* 缺 ko 警告）。
