...
| 3 | **KYB 分层**：注册/体验额度免 KYB（点击签约+邮箱验证+银行转账实名背书已够）；**提升信用额度（credit_limit_override）或未来大额累计充值必须先 KYB**（登記簿謄本/营业执照，admin 人工审核后置 `is_enterprise_verified=1`）——信用额度=平台授信，授信必须知道对方是谁 | 规则生效（admin 操作纪律）；上传+审核界面未开发，按需排期 |
