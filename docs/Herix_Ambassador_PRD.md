...
> **架构变更（2026-07-18）**：本节原结论"不需要 is_agency 标记"已被推翻——见 §28 入驻向导，`brand_profiles` 新增 `account_type`(`BRAND`/`AGENCY`) 字段，因为未来功能与文案需要按身份分支（如入驻引导语气、任务发布后的品牌方分享提示）。以下 23.1-23.2 原文保留作历史参考，实际以 §28 为准。

### 23.1 广告代理在生态中的角色（历史原文，account_type 之前）
...
| 角色 | 平台身份 | 核心用途 |
|------|---------|---------|
| 品牌 | BRAND 账号，account_type=BRAND | 公开任务为主 |
| 广告代理 | BRAND 账号，account_type=AGENCY | 定向任务为主，管理自己的赫使网络；代客户执行 |
