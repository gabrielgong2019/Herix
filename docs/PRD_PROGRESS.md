...
| 小程序微信账号体系（与邮箱统一） | ✅ | 2026-07-17：一个用户一行 users，openid/邮箱=同一身份的两把钥匙。wechat-login 静默登录(openid 走云托管 X-WX-OPENID 头,生产须配 WX_PROXY_SECRET 防伪造)→无账号选"一键注册"或"登录并绑定已有邮箱账号"；微信用户可补绑邮箱(BIND_EMAIL 验证码+设密码→可登网页版+收邮件)。冲突规则：一账号一微信/一微信一账号,不做合并。landing 一键进入+邮箱登录自动绑定。顺手修：邮箱大小写不一致(注册原样存/登录原样查,大写注册小写登录对不上)→统一小写存+LOWER()查；profile 注册表单漏验证码 UI(会被 CODE_REQUIRED 卡死)→补。测试 14/14+验证码回归 9/9 |
| i18n 反向同步 + 占位符校验 | ❌ | 按需触发（2026-07-17 定案）：当矩阵出现运营修改行（updated_by≠seed，当前两库均为 0）后再建——①反向同步脚本把矩阵改动导回代码 json 生成 diff；②seed/矩阵保存时校验 {placeholder} 拼写。触发时机：首次真人运营用矩阵改词条时 |
| 打款费率体系（阶梯+跨境锁汇率） | ✅ | 2026-07-17 拍板落地：payout_fee_rules(from/to国+币种,UNIQUE)——同国=阶梯固定费(JP→JP:≤5万¥300/以上¥500)，跨国=阶梯(JP→CN:≤1万¥500/≤5万¥800/以上¥1200)+150bps 汇率加点；申请时锁定中间价(platform_settings fx_mid_JPY_CNY 运营维护,接API前手动)；对称波动期望不亏(+50bps)，规模大上远期对冲。转出国 V1 恒 JP(钱包混池,分仓待多国实体)。提现申请快照 to_country/汇率/加点/目标币金额；提现页费用预览切服务端(删写死 FX_RATE)；admin定价页新增费率/汇率编辑；商家入驻加归属国(默认JP)。测试 9/9 |
