...
ratingsRouter.post('/:taskId', requireAuth, requireRole('HERALD'), (req: Request, res: Response) => {
  ...
  const rating = findOne('SELECT * FROM task_ratings WHERE id = ?', [id]);
  res.status(201).json(rating);
});

/** GET /api/ratings/:taskId — 查看任务的评价列表 */
ratingsRouter.get('/:taskId', (req: Request, res: Response) => {
  const ratings = findMany<any>(
