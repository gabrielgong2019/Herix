...
export async function seedIfEmpty(): Promise<void> {
  ...
  // ── 2. 品牌/Herald 资料 ──
  await pool.query("INSERT INTO brand_profiles (id, user_id, contact_name) VALUES ($1,$2,$3)",
    [genId(), uBrand, '周大福珠宝']);
  await pool.query("INSERT INTO brand_profiles (id, user_id, contact_name) VALUES ($1,$2,$3)",
    [genId(), uAdmin, 'Herix运营']);
  ...
  // ── 3. 品牌任务（周大福珠宝 7 个）──
  const brandTaskDefs: [string, string, number, number, number, string][] = [
    ['新品珠宝推广 #1', '在海外华人社群推广新款珠宝，需要拍摄佩戴照片并分享体验。', 3500, 400, 3, 'STANDARD'],
    ['新品珠宝推广 #2', '在海外华人社群推广新款珠宝。', 4000, 500, 3, 'STANDARD'],
    ['新品珠宝推广 #3', '在海外华人社群推广新款珠宝。', 4500, 600, 3, 'STANDARD'],
    ['Remitly 品牌大使', '分享 Remitly 汇款推荐码，每成功转化一位注册用户获得奖励。', 300000, 3000, 100, 'PERFORMANCE'],
    ['熊猫外卖拉新大使', '在中国社群分享熊猫外卖推荐码，新注册用户即算成功。', 200000, 2000, 100, 'PERFORMANCE'],
    ['母婴产品体验', '体验并分享母婴产品使用心得，需拍摄使用照片。', 200000, 20000, 3, 'STANDARD'],
    ['口红新品测评', '试色并分享口红新品，发布到小红书或Instagram。', 200000, 20000, 3, 'STANDARD'],
  ];
  const brandTaskIds: string[] = [];
  for (const [title, desc, budget, comm, maxH, mode] of brandTaskDefs) {
    const tid = genId();
    brandTaskIds.push(tid);
    await pool.query(
  ...
  // 索引: 0=珠宝#1, 1=珠宝#2, 2=珠宝#3, 3=Remitly, 4=熊猫外卖, 5=母婴, 6=口红

  // ── 4. Gabriel 的任务（4 个）──
  const gabrielTaskDefs: [string, string, number, number, number, string][] = [
    ['体验漂亮的房子', '拍摄并分享高端住宅体验，需入住体验并产出图文内容。', 200000, 20000, 3, 'STANDARD'],
    ['美食探店任务', '前往指定餐厅用餐并拍摄短视频，分享真实用餐体验。', 150000, 15000, 5, 'STANDARD'],
    ['Gabriel体验任务', '拍摄并分享你的生活方式，面向海外华人群体。', 100000, 10000, 3, 'STANDARD'],
    ['发小红书任务', '在小红书发布品牌体验笔记，需包含真实使用感受和高质量图片。', 80000, 8000, 5, 'STANDARD'],
  ];
  const gabrielTaskIds: string[] = [];
  for (const [title, desc, budget, comm, maxH, mode] of gabrielTaskDefs) {
    const tid = genId();
    gabrielTaskIds.push(tid);
    await pool.query(
      "INSERT INTO tasks (id, creator_id, mode, title, description, budget, commission, max_heralds, status, published_at, escrow_amount, is_escrowed, code_mode) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'OPEN',TO_CHAR(CURRENT_TIMESTAMP,'YYYY-MM-DD HH24:MI:SS'),$9,1,'auto')",
      [tid, uGabriel, mode, title, desc, budget, comm, maxH, comm * maxH]
    );
  }
  // 索引: 0=漂亮房子, 1=美食探店, 2=Gabriel体验, 3=发小红书

  // ── 5. Alice 报名 ──
  // 珠宝#1: APPROVED + 提交作品已通过
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id, status) VALUES ($1,$2,$3,'APPROVED')",
    [genId(), brandTaskIds[0], uAlice]);
  await pool.query("INSERT INTO task_submissions (id, task_id, herald_id, status, content_url, description) VALUES ($1,$2,$3,'APPROVED',$4,$5)",
    [genId(), brandTaskIds[0], uAlice, 'https://instagram.com/p/test', '完成了品牌体验拍摄，发布了3篇图文笔记']);
  // Remitly: APPROVED + 推广码
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id, status) VALUES ($1,$2,$3,'APPROVED')",
    [genId(), brandTaskIds[3], uAlice]);
  await pool.query("INSERT INTO ambassador_tasks (id, task_id, herald_id, unique_code, status) VALUES ($1,$2,$3,$4,'active')",
    [genId(), brandTaskIds[3], uAlice, genCode()]);
  // 母婴: APPROVED
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id, status) VALUES ($1,$2,$3,'APPROVED')",
    [genId(), brandTaskIds[5], uAlice]);
  // 熊猫外卖: PENDING
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id) VALUES ($1,$2,$3)",
    [genId(), brandTaskIds[4], uAlice]);

  // ── 6. Gabriel 报名 ──
  // 口红: APPROVED
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id, status) VALUES ($1,$2,$3,'APPROVED')",
    [genId(), brandTaskIds[6], uGabriel]);
  // Remitly: APPROVED + 推广码
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id, status) VALUES ($1,$2,$3,'APPROVED')",
    [genId(), brandTaskIds[3], uGabriel]);
  await pool.query("INSERT INTO ambassador_tasks (id, task_id, herald_id, unique_code, status) VALUES ($1,$2,$3,$4,'active')",
    [genId(), brandTaskIds[3], uGabriel, genCode()]);
  // 发小红书: PENDING（Gabriel 自己的任务）
  await pool.query("INSERT INTO task_applications (id, task_id, herald_id) VALUES ($1,$2,$3)",
    [genId(), gabrielTaskIds[3], uGabriel]);
  ...
  for (let i = 0; i < 3; i++) {
    await pool.query(
      ...
      [genId(), uBrand, brandTaskIds[i], brandTaskDefs[i][3] * brandTaskDefs[i][4], `任务 ${brandTaskDefs[i][0]} 资金托管`]
    );
  }
  // Alice 结算
  await pool.query(
  ...
  console.log('[seed] 测试账号 (密码: 123456):');
  console.log('[seed]   admin@herix.com (ADMIN)');
  console.log('[seed]   brand@d.com (BRAND)');
  console.log('[seed]   alice@d.com (HERALD)');
  console.log('[seed]   gabrielgong2019@outlook.com (HERALD+BRAND)');
}
